    ...
    <section className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row items-start gap-8 mb-12">
        ...
        <div>
          ...
          <p className="text-muted-foreground mb-4">
            我是 {config.hero_name} ({config.site_name})，{config.school}{config.major}专业学生，
            {config.role}。{config.hero_bio}
          </p>
          <div className="flex items-center gap-4">
            ...
            {config.email && (
              <a href={`mailto:${config.email}`} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
                <Mail size={14} /> Email
              </a>
            )}
          </div>
      ...
      <div className="mb-12">
        ...
        <div className="p-4 rounded-lg border border-border">
          ...
          <p className="text-sm text-muted-foreground">{config.major} | {config.grade}</p>
      ...
      <div>
        ...
        {Object.entries(grouped).length > 0 ? (
              ...
              <div className="space-y-3">
                {items.map(skill => (
                  <div key={skill.id} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground w-10 text-right">{skill.level}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <p className="text-muted-foreground">暂无技能数据，请在后台管理中添加。</p>
        )}
      </div>
    </section>
  )
